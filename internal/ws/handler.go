...
func HandleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	roomExists, err := store.RoomExists(r.Context(), msg.Room)
	...
	inRoom, err := store.IsUserInRoom(r.Context(), msg.Room, uid)
	if err != nil || !inRoom {
		log.Println("user is not a member of room:", msg.Room)
		_ = conn.Close()
		return
	}
	...
	// Атомарно заменяем старую сессию того же пользователя, если она зависла
	// из-за нестабильной сети/быстрых rejoin.
	replaced := room.UpsertUser(user)
	if replaced != nil {
		log.Printf("♻️ REPLACED stale session: user \"%s\" (id=%s) in room %s\n", prof.DisplayName, uid, msg.Room)
		replaced.Close()
	}
